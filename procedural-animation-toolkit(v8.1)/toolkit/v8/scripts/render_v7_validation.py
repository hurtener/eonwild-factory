#!/usr/bin/env python3
"""Render the baked V7 GLB and build a self-contained multi-clip validation HTML."""
...
class V7Renderer:
    def __init__(
        ...
        texture_path = output_dir / f"_v7_basecolor{extension}"
...
def render_preview(
    ...
    video = output_dir / f"v7_{spec.key}_{spec.fps}fps.mp4"
    ...
    montage = output_dir / f"v7_{spec.key}_keyframes.jpg"
...
def create_html(destination: Path, media: dict, specs: list[PreviewSpec], report: dict, fixture: dict, baked: dict | None) -> None:
    ...
    locomotion_clip = report_clips.get("PROC_WALK_RELAXED_V7_ROOTMOTION", {})
    locomotion_diag = locomotion_clip.get("diagnostics", {})
    terrain_diag = report_clips.get("PROC_WALK_UNEVEN_TERRAIN_V7_ROOTMOTION", {}).get("diagnostics", {})
    ...
    html = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Eonwild Tarbosaurus V7 validation</title>
<style>
...
</style></head><body><div class="wrap"><header><div><div class="eyebrow">Eonwild procedural animation toolkit · V7</div><h1>Tarbosaurus production animation pack</h1></div><div class="status">✓ generated on the real 75-joint rig</div></header>
...
<div class="cols"><section class="card"><h2>What V7 fixes and polishes</h2><p>V7 regenerates the entire pack from the contact solver after removing the V6 pelvis-export regression. The final pelvis transform is established first, both complete legs and weighted sole patches are solved against that exact transform, and those same local tracks are exported without a later balance offset.</p><ul><li>The relaxed walk is fitted to the supplied side-view reference at 0.64 Hz, with compact rear extension, progressive knee–ankle folding, early toe release, and deliberately small whole-body sway.</li><li>Vertex-level contact checks use 64 representative skinned sole vertices per foot; the baked walk and turns were re-sampled after export to reject hovering and ground penetration.</li><li>Left and right turns use true curved root paths, inside/outside step asymmetry, visible head and neck anticipation, delayed chest and pelvis commitment, and an opposing nine-joint tail counter-shape.</li><li>Terrain, slope, start, braking, alert movement, idle, eating, mirrored power bites, and roar are generated from the same pelvis-first contact architecture.</li><li>Eighteen named transition clips preserve root and pelvis endpoints for exact player handoff without a second crossfade.</li><li>The calibrated mostly-closed mouth and intentional jaw openings from V5/V6 are retained.</li></ul><div class="note">The red contact-debug preview shows the exact selected mesh vertices evaluated against the uneven terrain—not toe-bone proxy points.</div></section><section class="card"><h2>Automated validation</h2><ul><li class="pass">All 68 semantic tags resolve; no mapped source bone is missing.</li><li class="pass">75-joint skin and inverse-bind matrices are preserved.</li><li class="pass">Signed knee and ankle branch constraints are enforced across every generated locomotion and action clip.</li><li class="pass">Source animation names are unique and all channel targets are valid.</li><li class="pass">Every animation timeline is finite and strictly increasing.</li><li class="pass">The contact system evaluates at 120 Hz and exports at 60 Hz; exact baked-track regression tests then recheck feet, pelvis, turns, and transitions.</li></ul><h3>Human review still matters</h3><p>Judge mass, intent, facial readability, foot deformation, transition timing, tail damping, and whether each action fits the game camera. The validators prove structural and biomechanical constraints; they do not replace animation direction.</p><div class="note">Preview videos are baked from the generated GLB itself. The interactive browser demo in the toolkit plays the same baked animation tracks directly.</div></section></div>
<footer>V7 refinement and inherited capability flags: {sum(bool(v) for v in implementation.values())}/{len(implementation)} V7 capability groups enabled. The supplied reference video is embedded as a separate visual comparison and is not represented as calibrated mocap.</footer></div>
...
def main() -> int:
    ...
    generator = TarbosaurusV7LocomotionGenerator(source_asset, semantics, profile)
    pack = AnimationPackReader(asset)
    renderer = V7Renderer(asset, generator, args.width, args.height, output)

    cycle = 1.0 / profile.cycle_hz
    specs = [
        PreviewSpec("walk_side", "V7 contact-locked relaxed walk · side", "PROC_WALK_RELAXED_V7_INPLACE", "side", 60, cycle, "flat", False, "Reference-fitted 0.64 Hz gait. Review compact rear stance, continuous knee/ankle folding, small pelvis travel, planted soles and closed mouth."),
        PreviewSpec("walk_threeq", "V7 contact-locked relaxed walk · three-quarter", "PROC_WALK_RELAXED_V7_INPLACE", "threeq", 60, cycle, "flat", False, "The same final baked tracks from a gameplay-oriented camera."),
        PreviewSpec("turn_left_threeq", "Turn left · head/neck lead and tail counter-shape", "PROC_TURN_LEFT_35_V7_ROOTMOTION", "threeq", 60, None, "flat", False, "Head and neck anticipate the future path, chest follows, pelvis commits later, inside/outside steps diverge, and the tail counters."),
        PreviewSpec("turn_right_threeq", "Turn right · mirrored mass-led mechanics", "PROC_TURN_RIGHT_35_V7_ROOTMOTION", "threeq", 60, None, "flat", False, "Mirror review for turn asymmetry and anatomical leg continuity."),
        PreviewSpec("turn_left_overhead", "Turn left · overhead path and counterweight audit", "PROC_TURN_LEFT_35_V7_ROOTMOTION", "overhead", 60, None, "flat", False, "Overhead review exposes root arc, cranio-cervical lead, foot arcs and tail opposition."),
        PreviewSpec("idle", "Living-neutral idle", "PROC_IDLE_BREATH_V7", "threeq", 30, 6.0, "flat", False, "Quiet weight transfer without V6 lateral flight; calibrated mouth closure and subtle breathing."),
        PreviewSpec("eat", "Whole-body eating loop", "PROC_EAT_LOOP_V7", "threeq", 30, 8.0, "flat", False, "Pelvis, legs, torso, neck and jaw lower and recover as one grounded action."),
        PreviewSpec("bite_left", "Left-lead hindlimb-driven power bite", "PROC_BITE_ATTACK_V7", "threeq", 30, None, "flat", False, "Rear preload, leg drive, catch step, jaw closure at contact, recoil and grounded recovery."),
        PreviewSpec("bite_right", "Right-lead mirrored power bite", "PROC_BITE_ATTACK_MIRRORED_V7", "threeq", 30, None, "flat", False, "Independent mirrored lead-side validation."),
        PreviewSpec("bite_transition", "Walk → bite → walk · exact handoffs", ("PROC_WALK_RELAXED_V7_INPLACE", "PROC_WALK_TO_BITE_READY_V7", "PROC_BITE_ATTACK_V7", "PROC_BITE_TO_WALK_V7", "PROC_WALK_RELAXED_V7_INPLACE"), "threeq", 30, None, "flat", False, "Actual baked sequence with no second runtime crossfade."),
        PreviewSpec("terrain", "Uneven terrain · vertex sole adaptation", "PROC_WALK_UNEVEN_TERRAIN_V7_ROOTMOTION", "threeq", 60, None, "uneven", True, "Red witnesses are the selected skinned sole vertices used by the contact validator."),
    ]

    videos: dict[str, Path] = {}
    montages: list[tuple[PreviewSpec, Path]] = []
    for spec in specs:
        print(f"[render-v7] {spec.key}: {spec.label}", file=sys.stderr, flush=True)
    ...
    create_master_montage(montages, output / "tarbosaurus_v7_master_montage.jpg")

    report = json.loads(Path(args.report).read_text(encoding="utf-8"))
    fixture_path = output.parent / "v7-fixture-validation.json"
    fixture = json.loads(fixture_path.read_text()) if fixture_path.exists() else {}
    baked_path = output / "v7-baked-glb-validation.json"
    ...
    html_path = output / "OPEN_ME_tarbosaurus_v7_validation.html"
    create_html(html_path, media, specs, report, fixture, baked)
    shutil.rmtree(output / "_frames", ignore_errors=True)
    for temporary_texture in output.glob("_v7_basecolor.*"):
