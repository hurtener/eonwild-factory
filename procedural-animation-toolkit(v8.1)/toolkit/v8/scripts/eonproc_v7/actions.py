...
class TarbosaurusV7ActionGenerator:
    def __init__(self, locomotion: TarbosaurusV7LocomotionGenerator):
        ...
        jaw_index = self.asset.name_to_node[self.g.jaw_root]
        self.jaw_hinge_axis_local = self.asset.rest_world_rotation[jaw_index].inv().apply(
            self.g.base_basis.lateral
        )
        self.jaw_hinge_axis_local /= max(np.linalg.norm(self.jaw_hinge_axis_local), 1e-12)
    ...
    def _pose(
        ...
        # The jaw hinge belongs to the jaw's rest-local frame. Re-projecting a
        # moving world lateral axis after head yaw/roll creates the visibly
        # off-centre V8 jaw. Keep one anatomical local hinge instead.
        pose.rotate_local(g.jaw_root, self.jaw_hinge_axis_local, jaw)
    ...
    def _generate(
        ...
        for time_value in internal_times:
            ...
            for side in ("l", "r"):
                ...
                foot_contact_state[side].append({"stance": bool(solved[side]["foot"].stance), "load": float(solved[side]["foot"].load), "target_up_m": float(np.dot(solved[side]["foot"].target_world-self.g.foot_rest[side],self.g.base_basis.up))})
        ...
        diagnostics = {
            ...
            "max_sole_penetration_sample": {side: {"phase": float(int(np.argmax(values))/max(internal_count-1,1)), "time_seconds": float(internal_times[int(np.argmax(values))]), **foot_contact_state[side][int(np.argmax(values))]} for side,values in penetration.items()},
            "pelvis_translation": {
