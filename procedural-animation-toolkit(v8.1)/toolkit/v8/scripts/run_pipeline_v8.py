#!/usr/bin/env python3
"""Generate the complete Eonwild Tarbosaurus V8.1 animation pack."""
...
def main() -> int:
    ...
    parser.add_argument("--profile", required=True, help="Canonical V8.1 profile JSON; defaults are forbidden for releases")
    ...
    pivot_meta = lower_foot_pivots(asset, semantics, basis, profile.foot_pivot_gap_fraction)
    jaw_meta = None
    ...
    pivot_path = Path(args.output_dir) / "foot-pivot-calibration-v8.1.json"
    pivot_path.write_text(json.dumps(pivot_meta, indent=2), encoding="utf-8")
    outputs["footPivots"] = pivot_path
    if jaw_meta is not None:
        jaw_path = Path(args.output_dir) / "jaw-calibration-v8.1.json"
