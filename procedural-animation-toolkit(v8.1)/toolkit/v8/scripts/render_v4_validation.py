#!/usr/bin/env python3
"""Render the baked V4 GLB and build a self-contained multi-clip validation HTML."""
...
class V4Renderer:
    def __init__(
        ...
        texture_path = output_dir / f"_v4_basecolor{extension}"
...
def render_preview(
    ...
    video = output_dir / f"v4_{spec.key}_{spec.fps}fps.mp4"
    ...
    montage = output_dir / f"v4_{spec.key}_keyframes.jpg"
...
def create_html(destination: Path, media: dict, specs: list[PreviewSpec], report: dict, fixture: dict, baked: dict | None) -> None:
    ...
    locomotion_clip = report_clips.get("PROC_WALK_RELAXED_V4_ROOTMOTION", {})
    locomotion_diag = locomotion_clip.get("diagnostics", {})
    terrain_diag = report_clips.get("PROC_WALK_UNEVEN_TERRAIN_V4_ROOTMOTION", {}).get("diagnostics", {})
    ...
    html = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Eonwild Tarbosaurus V4 validation</title>
<style>
...
</style></head><body><div class="wrap"><header><div><div class="eyebrow">Eonwild procedural animation toolkit · V4</div><h1>Tarbosaurus production animation pack</h1></div><div class="status">✓ generated on the real 75-joint rig</div></header>
...
<div class="cols"><section class="card"><h2>What V4 adds</h2><p>The relaxed walk is fitted to the supplied Tarbosaurus side-view reference for cadence and posture, while remaining a procedural, constraint-driven animation rather than pretending a single uncalibrated video is full motion capture.</p><ul><li>Vertex-level contact checks use 64 representative skinned sole vertices per foot in the validated browser-production profile.</li><li>Terrain clips solve both surface height and local normal, then adapt foot orientation, leg reach, pelvis height, roll, and pitch.</li><li>Curved root paths author left/right turns with inside/outside stride asymmetry.</li><li>Start and braking schedules derive phase, speed, acceleration, body lean, support timing, and root distance from the same time series.</li><li>Alert locomotion changes cadence, step height, head carriage, scanning, tail carriage, and limb tension.</li><li>Eating, bite, and roar actions use the complete neck, spine, jaw, pelvis, arms, and dynamic tail rather than moving only the head.</li><li>Sixteen named transition clips bridge locomotion, alert, eating, bite, roar, and idle states.</li></ul><div class="note">The red contact-debug preview shows the exact selected mesh vertices evaluated against the uneven terrain—not toe-bone proxy points.</div></section><section class="card"><h2>Automated validation</h2><ul><li class="pass">All 68 semantic tags resolve; no mapped source bone is missing.</li><li class="pass">75-joint skin and inverse-bind matrices are preserved.</li><li class="pass">Signed knee and ankle branch constraints remain inherited from V3.</li><li class="pass">Source animation names are unique and all channel targets are valid.</li><li class="pass">Every animation timeline is finite and strictly increasing.</li><li class="pass">The validated browser-production pack is solved and exported at 60 Hz; the toolkit also includes a 120 Hz ultra-offline profile.</li></ul><h3>Human review still matters</h3><p>Judge mass, intent, facial readability, foot deformation, transition timing, tail damping, and whether each action fits the game camera. The validators prove structural and biomechanical constraints; they do not replace animation direction.</p><div class="note">Preview videos are baked from the generated GLB itself. The interactive browser demo in the toolkit plays the same baked animation tracks directly.</div></section></div>
<footer>Implementation flags: {sum(bool(v) for v in implementation.values())}/{len(implementation)} V4 capability groups enabled. The supplied reference video is embedded as a separate visual comparison and is not represented as calibrated mocap.</footer></div>
...
def main() -> int:
    ...
    generator = TerrainAwareLocomotionGenerator(source_asset, semantics, profile)
    pack = AnimationPackReader(asset)
    renderer = V4Renderer(asset, generator, args.width, args.height, output)

    cycle = 1.0 / profile.cycle_hz
    alert_cycle = 1.0 / profile.alert_cycle_hz
    specs = [
        PreviewSpec("walk_side", "Relaxed reference-fitted walk · side", "PROC_WALK_RELAXED_V4_INPLACE", "side", 60, cycle, "flat", False, "One complete relaxed gait cycle. Review stance duration, head horizon, support shift, and tail compliance."),
        PreviewSpec("walk_threeq", "Relaxed reference-fitted walk · 3/4", "PROC_WALK_RELAXED_V4_INPLACE", "threeq", 60, cycle, "flat", False, "Three-quarter mass and balance review of the same exact baked clip."),
        PreviewSpec("terrain", "Uneven terrain adaptation", "PROC_WALK_UNEVEN_TERRAIN_V4_ROOTMOTION", "threeq", 60, cycle / 0.94, "uneven", False, "Root-motion clip solving terrain height, normal, pelvis frame, legs, feet, toes, and skinned sole clearance."),
        PreviewSpec("sole_debug", "Vertex-level sole collision debug", "PROC_WALK_UNEVEN_TERRAIN_V4_ROOTMOTION", "feet", 30, cycle / 0.94, "uneven", True, "Red points are the selected actual skinned sole vertices used by the collision validator."),
        PreviewSpec("upslope", "Upslope height + normal adaptation", "PROC_WALK_UPSLOPE_V4_ROOTMOTION", "side", 30, cycle / 0.92, "slope", False, "The solver changes foot pitch, leg reach, pelvis height and body compensation on a 7.5% slope."),
        PreviewSpec("turn", "Authored 35° left turn", "PROC_TURN_LEFT_35_V4_ROOTMOTION", "overhead", 30, None, "flat", False, "Curved root path with inside/outside stride asymmetry, heading change, body lean, and contact persistence."),
        PreviewSpec("start", "Idle-to-walk acceleration", "PROC_START_WALK_V4_ROOTMOTION", "side", 30, None, "flat", False, "Minimum-jerk speed ramp with phase-safe support reorganization, acceleration lean, and resolved root distance."),
        PreviewSpec("brake", "Walk-to-idle braking", "PROC_BRAKE_TO_IDLE_V4_ROOTMOTION", "side", 30, None, "flat", False, "Deceleration schedule with braking posture, shortening steps, planted contacts, and settle into support."),
        PreviewSpec("alert", "Alert locomotion", "PROC_ALERT_WALK_V4_INPLACE", "threeq", 60, alert_cycle, "flat", False, "Raised head and neck, scanning attention, altered cadence/stride, higher swing clearance, and more active tail carriage."),
        PreviewSpec("eat", "Feeding / eating loop", "PROC_EAT_LOOP_V4", "threeq", 30, None, "flat", False, "Eight-second loop with body lowering, load redistribution, neck reach, search motion, four bite cycles, chewing, and tail balance."),
        PreviewSpec("bite", "Bite attack", "PROC_BITE_ATTACK_V4", "side", 60, None, "flat", False, "Anticipation, crouch, jaw preparation, neck thrust, lunge, snap, recoil, and full-body recovery."),
        PreviewSpec("roar", "Roar display", "PROC_ROAR_V4", "threeq", 60, None, "flat", False, "Inhale, chest/neck preparation, large jaw gape, head vibration, tail brace, release, and neutral recovery."),
        PreviewSpec("bite_transition", "Walk → bite → walk transition reel", ("PROC_WALK_RELAXED_V4_INPLACE", "PROC_WALK_TO_BITE_READY_V4", "PROC_BITE_ATTACK_V4", "PROC_BITE_TO_WALK_V4", "PROC_WALK_RELAXED_V4_INPLACE"), "threeq", 30, None, "flat", False, "The exact baked locomotion/action bridges around the one-shot bite; no hidden editor crossfade."),
    ]

    videos: dict[str, Path] = {}
    montages: list[tuple[PreviewSpec, Path]] = []
    for spec in specs:
        print(f"[render-v4] {spec.key}: {spec.label}", file=sys.stderr, flush=True)
    ...
    create_master_montage(montages, output / "tarbosaurus_v4_master_montage.jpg")

    report = json.loads(Path(args.report).read_text(encoding="utf-8"))
    fixture_path = output.parent / "v4-fixture-validation.json"
    fixture = json.loads(fixture_path.read_text()) if fixture_path.exists() else {}
    baked_path = output / "v4-baked-glb-validation.json"
    ...
    if args.reference_video:
        reference_path = output / "reference_tarbosaurus_walk.mp4"
        shutil.copy2(args.reference_video, reference_path)
        media["reference"] = data_uri(reference_path, "video/mp4")
        specs.append(PreviewSpec("reference", "Supplied Tarbosaurus reference", "SUPPLIED_REFERENCE_VIDEO", "source", 24, None, "flat", False, "The user-supplied side-view quality target. It is used for cadence and posture fitting, not represented as calibrated motion capture."))
    if args.v3_side:
        ...
        specs.append(PreviewSpec("v3_side", "V3 baseline · side", "PROC_WALK_LARGE_V3_INPLACE", "side", 60, None, "flat", False, "Previous validated V3 walk for direct visual regression review."))
    if args.v3_threeq:
        ...
        specs.append(PreviewSpec("v3_threeq", "V3 baseline · 3/4", "PROC_WALK_LARGE_V3_INPLACE", "threeq", 60, None, "flat", False, "Previous validated V3 three-quarter view for mass, posture, and tail comparison."))
    html_path = output / "OPEN_ME_tarbosaurus_v4_validation.html"
