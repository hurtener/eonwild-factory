"""End-to-end Tarbosaurus V8 animation factory."""
...
from .actions import TarbosaurusV8ActionGenerator
from .locomotion import TarbosaurusV8LocomotionGenerator
from .profile import BipedV8Profile
from .transitions import pose_transition, phase_matched_loop_transition


class TarbosaurusV8Generator:
    def __init__(self, asset: GlbAsset, semantics: SemanticMap, profile: BipedV8Profile):
        self.asset = asset
        self.semantics = semantics
        self.profile = profile
        self.locomotion = TarbosaurusV8LocomotionGenerator(asset, semantics, profile)
        self.actions = TarbosaurusV8ActionGenerator(self.locomotion)
        self.clips: list[AnimationClip] = []
        self.report: dict[str, Any] = {}

    def _add_locomotion(self, schedule):
        started = time.time()
        print(f"[v8] generating {schedule.name}", file=sys.stderr, flush=True)
        result = self.locomotion.generate_schedule(schedule, keep_world_matrices=False)
        elapsed = time.time() - started
        print(f"[v8] completed {schedule.name} in {elapsed:.1f}s", file=sys.stderr, flush=True)
    ...
    def generate_all(self) -> list[AnimationClip]:
        ...
        walk_root, walk_inplace = self._add_locomotion(constant_schedule(
            "PROC_WALK_RELAXED_V8_1_ROOTMOTION",
            g,
            cycles=2.0,
            terrain=flat,
            cycle_hz=p.cycle_hz,
            stride_scale=1.0,
            loop=True,
            tags=("walk", "relaxed", "reference_fitted", "contact_locked", "v8"),
        ))
        ...
        terrain_root, terrain_inplace = self._add_locomotion(constant_schedule(
            "PROC_WALK_UNEVEN_TERRAIN_V8_1_ROOTMOTION",
            g,
            cycles=2.0,
            terrain=uneven,
        ...
        slope_root, slope_inplace = self._add_locomotion(constant_schedule(
            "PROC_WALK_UPSLOPE_V8_1_ROOTMOTION",
            g,
            cycles=2.0,
            terrain=slope,
        ...
        turn_left_root, turn_left_inplace = self._add_locomotion(constant_schedule(
            "PROC_TURN_LEFT_35_V8_1_ROOTMOTION",
            g,
            cycles=2.0,
        ...
        turn_right_root, turn_right_inplace = self._add_locomotion(constant_schedule(
            "PROC_TURN_RIGHT_35_V8_1_ROOTMOTION",
            g,
            cycles=2.0,
            path_factory=arc_factory(-1.0),
            terrain=flat,
        ...
        start_root, start_inplace = self._add_locomotion(speed_transition_schedule(
            "PROC_START_WALK_V8_1_ROOTMOTION",
            g,
        ...
        stop_root, stop_inplace = self._add_locomotion(speed_transition_schedule(
            "PROC_BRAKE_TO_IDLE_V8_1_ROOTMOTION",
            g,
        ...
        alert_root, alert_inplace = self._add_locomotion(constant_schedule(
            "PROC_ALERT_WALK_V8_1_ROOTMOTION",
            g,
            cycles=2.0,
            terrain=flat,
            alert_amount=1.0,
        ...
        action_factories = (
            ...
            ("power_left", lambda: self.actions.power_attack("l")),
            ("power_right", lambda: self.actions.power_attack("r")),
            ("roar", self.actions.roar),
        )
        action_clips: list[AnimationClip] = []
        for label, factory in action_factories:
            started = time.time()
            print(f"[v8] generating action {label}", file=sys.stderr, flush=True)
            action_clips.append(factory())
            print(f"[v8] completed action {label} in {time.time() - started:.1f}s", file=sys.stderr, flush=True)
        idle, alert_idle, eat, bite_left, bite_right, power_left, power_right, roar = action_clips
        ...
        transitions = [
            pose_transition(
                "PROC_IDLE_TO_WALK_V8_1", idle, walk_inplace,
                1.75, p.export_sample_hz, 0.0, 0.0, root,
                ("transition", "idle", "walk", "pelvis_continuity"),
            ),
            pose_transition(
                "PROC_WALK_TO_IDLE_V8_1", walk_inplace, idle,
                1.75, p.export_sample_hz, 0.50, 0.0, root,
                ("transition", "walk", "idle", "pelvis_continuity"),
            ),
            phase_matched_loop_transition(
                "PROC_WALK_TO_ALERT_WALK_V8_1", walk_inplace, alert_inplace,
                1.0 / p.cycle_hz, p.export_sample_hz, root,
                ("transition", "walk", "alert", "phase_matched", "pelvis_continuity"),
            ),
            phase_matched_loop_transition(
                "PROC_ALERT_WALK_TO_WALK_V8_1", alert_inplace, walk_inplace,
                1.0 / p.cycle_hz, p.export_sample_hz, root,
                ("transition", "alert", "walk", "phase_matched", "pelvis_continuity"),
            ),
            pose_transition(
                "PROC_IDLE_TO_ALERT_IDLE_V8_1", idle, alert_idle,
                1.30, p.export_sample_hz, 0.0, 0.0, root,
                ("transition", "idle", "alert_idle"),
            ),
            pose_transition(
                "PROC_ALERT_IDLE_TO_IDLE_V8_1", alert_idle, idle,
                1.30, p.export_sample_hz, 0.0, 0.0, root,
                ("transition", "alert_idle", "idle"),
            ),
            pose_transition(
                "PROC_IDLE_TO_EAT_V8_1", idle, eat,
                2.40, p.export_sample_hz, 0.0, 0.0, root,
                ("transition", "idle", "eat", "whole_body_lower"),
            ),
            pose_transition(
                "PROC_EAT_TO_IDLE_V8_1", eat, idle,
                2.40, p.export_sample_hz, 0.0, 0.0, root,
                ("transition", "eat", "idle", "whole_body_rise"),
            ),
            pose_transition(
                "PROC_IDLE_TO_ROAR_READY_V8_1", idle, roar,
                0.95, p.export_sample_hz, 0.0, 0.0, root,
                ("transition", "idle", "roar"),
            ),
            pose_transition(
                "PROC_ROAR_TO_IDLE_V8_1", roar, idle,
                1.50, p.export_sample_hz, 1.0, 0.0, root,
                ("transition", "roar", "idle", "recovery"),
            ),
            pose_transition(
                "PROC_WALK_TO_BITE_READY_V8_1", walk_inplace, bite_left,
                1.15, p.export_sample_hz, 0.50, 0.0, root,
                ("transition", "walk", "bite", "lead_left", "target_acquisition"),
            ),
            pose_transition(
                "PROC_BITE_TO_WALK_V8_1", bite_left, walk_inplace,
                1.80, p.export_sample_hz, 1.0, 0.0, root,
                ("transition", "bite", "walk", "lead_left", "recovery"),
            ),
            pose_transition(
                "PROC_WALK_TO_BITE_READY_MIRRORED_V8_1", walk_inplace, bite_right,
                1.15, p.export_sample_hz, 0.0, 0.0, root,
                ("transition", "walk", "bite", "lead_right", "target_acquisition"),
            ),
            pose_transition(
                "PROC_BITE_MIRRORED_TO_WALK_V8_1", bite_right, walk_inplace,
                1.80, p.export_sample_hz, 1.0, 0.50, root,
                ("transition", "bite", "walk", "lead_right", "recovery"),
            ),
            pose_transition(
                "PROC_WALK_TO_POWER_ATTACK_V8_1", walk_inplace, power_left,
                0.42, p.export_sample_hz, 0.0, 0.0, root,
                ("transition", "walk", "power_attack", "support_phase_aware", "lead_left", "no_repeated_coil"),
            ),
            pose_transition(
                "PROC_WALK_TO_POWER_ATTACK_MIRRORED_V8_1", walk_inplace, power_right,
                0.42, p.export_sample_hz, 0.50, 0.0, root,
                ("transition", "walk", "power_attack", "support_phase_aware", "lead_right", "no_repeated_coil"),
            ),
            pose_transition(
                "PROC_POWER_ATTACK_TO_EAT_V8_1", power_left, eat,
                0.82, p.export_sample_hz, 1.0, 0.0, root,
                ("transition", "power_attack", "eat", "feeding_ready", "root_pelvis_continuity"),
            ),
            pose_transition(
                "PROC_POWER_ATTACK_MIRRORED_TO_EAT_V8_1", power_right, eat,
                0.82, p.export_sample_hz, 1.0, 0.0, root,
                ("transition", "power_attack", "eat", "feeding_ready", "root_pelvis_continuity"),
            ),
            pose_transition(
                "PROC_POWER_ATTACK_TO_WALK_V8_1", power_left, walk_inplace,
                1.20, p.export_sample_hz, 1.0, 0.0, root,
                ("transition", "power_attack", "walk", "arrested_recovery"),
            ),
            pose_transition(
                "PROC_WALK_TO_ROAR_READY_V8_1", walk_inplace, roar,
                1.35, p.export_sample_hz, 0.50, 0.0, root,
                ("transition", "walk", "roar"),
            ),
            pose_transition(
                "PROC_ROAR_TO_WALK_V8_1", roar, walk_inplace,
                1.90, p.export_sample_hz, 1.0, 0.0, root,
                ("transition", "roar", "walk", "recovery"),
            ),
            pose_transition(
                "PROC_WALK_TO_EAT_V8_1", walk_inplace, eat,
                2.45, p.export_sample_hz, 0.50, 0.0, root,
                ("transition", "walk", "eat", "settle"),
            ),
            pose_transition(
                "PROC_EAT_TO_WALK_V8_1", eat, walk_inplace,
                2.45, p.export_sample_hz, 0.0, 0.0, root,
        ...
        if len(names) != len(set(names)):
            ...
            raise ValueError(f"Duplicate V8 clip names: {duplicates}")
        ...
        critical = [
            ...
            if gates and (
                ...
                or any(token in name for token in ("IDLE_BREATH", "ALERT_IDLE", "EAT_LOOP", "NORMAL_ATTACK", "POWER_ATTACK", "ROAR_V8_1"))
                or "_TO_" in name
            )
        ]
        ...
        self.report = {
            ...
            "implementation": {
                ...
                "pronounced_cranio_cervical_turn_lead": True,
                "turn_head_overshoot_and_focus_pitch": True,
                ...
                "irregular_bite_hold_pull_chew_feeding": True,
                "hindlimb_driven_bite_left_right": True,
                "grounded_normal_attack_preserved": True,
                "airborne_one_foot_power_attack": True,
                "power_attack_feeding_handoff": True,
                "roar": True,
    ...
    def _event_markers(self, clip: AnimationClip) -> list[dict[str, Any]]:
        ...
        elif "POWER_ATTACK" in name and not clip.extras.get("transition"):
            events.extend([
                {"timeSeconds": duration * self.profile.power_coil_end_fraction, "type": "launch_leg_loaded"},
                {"timeSeconds": duration * self.profile.power_takeoff_fraction, "type": "takeoff"},
                {"timeSeconds": duration * self.profile.power_apex_fraction, "type": "apex"},
                {"timeSeconds": duration * self.profile.power_bite_contact_fraction, "type": "bite_contact", "windowSeconds": 0.05},
                {"timeSeconds": duration * self.profile.power_land_fraction, "type": "landing"},
                {"timeSeconds": duration * self.profile.power_catch_fraction, "type": "second_foot_catch"},
                {"timeSeconds": duration * self.profile.power_arrest_end_fraction, "type": "weight_arrest"},
                {"timeSeconds": duration, "type": "feeding_ready"},
            ])
        elif "NORMAL_ATTACK" in name:
            events.extend([
                {"timeSeconds": duration * self.profile.bite_focus_fraction, "type": "attack_target_locked"},
                {"timeSeconds": duration * self.profile.bite_anticipation_fraction, "type": "attack_hindlimb_release"},
                {"timeSeconds": duration * self.profile.bite_first_catch_fraction, "type": "attack_first_catch"},
                {"timeSeconds": duration * self.profile.bite_contact_fraction, "type": "bite_contact", "windowSeconds": 0.08},
                {"timeSeconds": duration * self.profile.bite_hold_end_fraction, "type": "bite_tear_release"},
                {"timeSeconds": duration * self.profile.bite_recovery_fraction, "type": "recovery_start"},
            ])
        elif name == "PROC_ROAR_V8_1":
            events.extend([
        ...
        elif name == "PROC_EAT_LOOP_V8_1":
            for index, phase in enumerate(self.profile.eating_bite_times):
                events.extend([
                    {"timeSeconds": duration * max(0.0, phase - 0.060), "type": "feeding_gape", "index": index},
                    {"timeSeconds": duration * phase, "type": "feeding_bite_contact", "index": index},
                    {"timeSeconds": duration * min(1.0, phase + 0.085), "type": "feeding_pull", "index": index, "direction": self.profile.eating_bite_directions[index]},
                    {"timeSeconds": duration * min(1.0, phase + 0.215), "type": "feeding_chew_swallow", "index": index},
                ])
    ...
    def manifest(self, asset_name: str) -> dict[str, Any]:
        return {
            "schema": "eonwild.animation-manifest.v8.1",
            "version": "8.1.0",
            "asset": asset_name,
            ...
            "runtimeContracts": {
                ...
                "turning": "eyes/head anticipate strongly; neck bends into target; chest follows; pelvis commits later; tail countershapes and settles",
                "normalAttack": "preserved grounded V8 two-step bite",
                "powerAttack": "gaze -> one-foot load -> coil -> takeoff -> airborne bite -> ordered landing -> arrest -> feeding-ready",
                "feeding": "whole-body low posture with irregular late-gape bites, clamped side-pulls, chewing and swallowing",
            },
            "clips": [
                {
                    "name": clip.name,
            ...
            "stateGraph": {
                "initial": "idle",
                "states": {
                    "idle": {"clip": "PROC_IDLE_BREATH_V8_1"},
                    "walk": {"clip": "PROC_WALK_RELAXED_V8_1_INPLACE", "speedMps": self.locomotion.speed},
                    "alert_idle": {"clip": "PROC_ALERT_IDLE_V8_1"},
                    "alert_walk": {"clip": "PROC_ALERT_WALK_V8_1_INPLACE"},
                    "eat": {"clip": "PROC_EAT_LOOP_V8_1"},
                },
                "oneShots": {
                    "normal_attack_left": {"clip": "PROC_NORMAL_ATTACK_V8_1", "priority": 70},
                    "normal_attack_right": {"clip": "PROC_NORMAL_ATTACK_MIRRORED_V8_1", "priority": 70},
                    "power_attack_left": {"clip": "PROC_POWER_ATTACK_V8_1", "priority": 90},
                    "power_attack_right": {"clip": "PROC_POWER_ATTACK_MIRRORED_V8_1", "priority": 90},
                    "roar": {"clip": "PROC_ROAR_V8_1", "priority": 60},
                },
                "actionSequences": {
                    "normal_attack_left": ["PROC_WALK_TO_BITE_READY_V8_1", "PROC_NORMAL_ATTACK_V8_1", "PROC_BITE_TO_WALK_V8_1", "PROC_WALK_RELAXED_V8_1_INPLACE"],
                    "normal_attack_right": ["PROC_WALK_TO_BITE_READY_MIRRORED_V8_1", "PROC_NORMAL_ATTACK_MIRRORED_V8_1", "PROC_BITE_MIRRORED_TO_WALK_V8_1", "PROC_WALK_RELAXED_V8_1_INPLACE"],
                    "power_attack_left_to_eat": ["PROC_WALK_TO_POWER_ATTACK_V8_1", "PROC_POWER_ATTACK_V8_1", "PROC_POWER_ATTACK_TO_EAT_V8_1", "PROC_EAT_LOOP_V8_1"],
                    "power_attack_right_to_eat": ["PROC_WALK_TO_POWER_ATTACK_MIRRORED_V8_1", "PROC_POWER_ATTACK_MIRRORED_V8_1", "PROC_POWER_ATTACK_MIRRORED_TO_EAT_V8_1", "PROC_EAT_LOOP_V8_1"],
                    "roar": ["PROC_WALK_TO_ROAR_READY_V8_1", "PROC_ROAR_V8_1", "PROC_ROAR_TO_WALK_V8_1", "PROC_WALK_RELAXED_V8_1_INPLACE"],
                    "eat": ["PROC_WALK_TO_EAT_V8_1", "PROC_EAT_LOOP_V8_1", "PROC_EAT_TO_WALK_V8_1", "PROC_WALK_RELAXED_V8_1_INPLACE"],
                    "alert": ["PROC_WALK_TO_ALERT_WALK_V8_1", "PROC_ALERT_WALK_V8_1_INPLACE"],
                },
            },
        }
    ...
    def write(self, output_dir: str | Path, source_path: str | Path, bone_map_path: str | Path):
        ...
        if not self.clips:
            self.generate_all()
        glb_path = output / "tarbosaurus_procedural_v8_1_animation_pack.glb"
        self.asset.write_animations(glb_path, self.animation_specs(), replace_existing=True)
        manifest = self.manifest(glb_path.name)
        manifest_path = output / "animation-manifest.v8.1.json"
        manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
        profile_path = output / "resolved-profile-v8.1.json"
        ...
        report_path = output / "procedural-v8.1-report.json"
