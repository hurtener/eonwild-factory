...
class TarbosaurusV8LocomotionGenerator(TarbosaurusV7LocomotionGenerator):
    ...
    def generate_schedule(self, schedule: LocomotionSchedule, keep_world_matrices: bool=True) -> LocomotionResult:
        ...
        jaw=self.jaw_root; ji=self.asset.name_to_node[jaw]
        axis=self.asset.rest_world_rotation[ji].inv().apply(self.base_basis.lateral); axis/=max(np.linalg.norm(axis),1e-12)
        for clip in (result.clip,result.in_place_clip):
            if clip is None: continue
            rv=(self.asset.rest_rotation[ji].inv()*Rotation.from_quat(clip.rotations[jaw])).as_rotvec()
            off=rv-np.outer(rv@axis,axis); off_deg=float(np.degrees(np.max(np.linalg.norm(off,axis=1))))
            clip.diagnostics["jaw_hinge_v8_1"]={"space":"jaw rest-local","axis_local":axis.tolist(),"maximum_off_hinge_degrees":off_deg}
