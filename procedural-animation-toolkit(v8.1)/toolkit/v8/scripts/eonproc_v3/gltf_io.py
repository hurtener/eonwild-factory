"""Minimal GLB 2.0 I/O used by the Eonwild procedural animation skill v3.
...
class GlbAsset:
    ...
    def _replace_accessor_values(self, accessor_index: int, values: np.ndarray) -> None:
        accessor=self.json["accessors"][accessor_index]; view=self.json["bufferViews"][accessor["bufferView"]]
        if accessor.get("sparse"): raise NotImplementedError("Sparse accessor replacement unsupported")
        dtype=_COMPONENT_DTYPES[accessor["componentType"]]; width=_TYPE_WIDTH[accessor["type"]]; count=int(accessor["count"]); array=np.asarray(values)
        if accessor["type"].startswith("MAT"):
            side=int(math.sqrt(width)); storage=array.transpose(0,2,1).reshape((count,width))
        else: storage=array.reshape((count,width))
        storage=np.asarray(storage,dtype=dtype); stride=int(view.get("byteStride",dtype.itemsize*width)); row_bytes=dtype.itemsize*width; base=int(view.get("byteOffset",0))+int(accessor.get("byteOffset",0)); binary=bytearray(self.binary)
        for i,row in enumerate(storage):
            start=base+i*stride; binary[start:start+row_bytes]=row.tobytes(order="C")
        self.binary=bytes(binary)

    def _refresh_rest_pose(self) -> None:
        self.rest_translation=[]; self.rest_rotation=[]; self.rest_scale=[]; self.rest_local=[]
        for node in self.nodes:
            if "matrix" in node:
                matrix=np.asarray(node["matrix"],dtype=np.float64).reshape(4,4).T; translation=matrix[:3,3].copy(); basis=matrix[:3,:3]; scale=np.linalg.norm(basis,axis=0); rotation=Rotation.from_matrix(basis@np.diag(1.0/scale))
            else:
                translation=np.asarray(node.get("translation",[0,0,0]),dtype=np.float64); rotation=Rotation.from_quat(node.get("rotation",[0,0,0,1])); scale=np.asarray(node.get("scale",[1,1,1]),dtype=np.float64); matrix=matrix_from_trs(translation,rotation,scale)
            self.rest_translation.append(translation); self.rest_rotation.append(rotation); self.rest_scale.append(scale); self.rest_local.append(matrix)
        self.rest_world=self.world_matrices(self.rest_local); self.rest_world_rotation=self.world_rotations(self.rest_rotation)

    def move_joint_world_position_preserve_rest_skin(self,joint_name:str,target_world_position:np.ndarray,skin_index:int=0)->dict[str,Any]:
        joint_index=self.name_to_node[joint_name]; child_indices=list(self.nodes[joint_index].get("children",[])); old_world=[m.copy() for m in self.rest_world]; old_position=old_world[joint_index][:3,3].copy(); target=np.asarray(target_world_position,dtype=np.float64).reshape(3); parent_index=self.parents[joint_index]; parent_world=np.eye(4) if parent_index is None else old_world[parent_index]
        new_local=(np.linalg.inv(parent_world)@np.append(target,1))[:3]; self.nodes[joint_index]["translation"]=new_local.tolist(); new_joint_world=parent_world@matrix_from_trs(new_local,self.rest_rotation[joint_index],self.rest_scale[joint_index]); inv_new=np.linalg.inv(new_joint_world); preserved={}
        for child_index in child_indices:
            name=self.nodes[child_index].get("name",f"node_{child_index}"); old_pos=old_world[child_index][:3,3].copy(); self.nodes[child_index]["translation"]=(inv_new@np.append(old_pos,1))[:3].tolist(); preserved[name]=old_pos.tolist()
        skin=self.json["skins"][skin_index]; joints=list(map(int,skin["joints"])); slot=joints.index(joint_index); inverse_bind=self.accessor(skin["inverseBindMatrices"]).astype(np.float64); inverse_bind[slot]=inv_new@old_world[joint_index]@inverse_bind[slot]; self._replace_accessor_values(skin["inverseBindMatrices"],inverse_bind); self._refresh_rest_pose()
        return {"joint":joint_name,"oldWorldPosition":old_position.tolist(),"newWorldPosition":self.rest_world[joint_index][:3,3].tolist(),"preservedChildWorldPositions":preserved,"skinJointSlot":slot}
