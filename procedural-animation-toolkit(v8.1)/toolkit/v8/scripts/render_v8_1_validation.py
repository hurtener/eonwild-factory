...
def render_preview(
    ...
    video = output_dir / f"v8_1_{spec.key}_{spec.fps}fps.mp4"
    ...
    montage = output_dir / f"v8_1_{spec.key}_keyframes.jpg"
...
def main() -> int:
    ...
    renderer = V81Renderer(asset, generator, args.width, args.height, output)

    cycle = 1.0 / profile.cycle_hz
    specs = [
        PreviewSpec("idle_side", "V8.1 accepted asymmetric idle · side", "PROC_IDLE_BREATH_V8_1", "side", 20, 6.0, "flat", False, "V5.5 chest/neutral chain and 66/34 staggered support on the V8 rig."),
        PreviewSpec("walk_side", "V8.1 relaxed walk · side", "PROC_WALK_RELAXED_V8_1_INPLACE", "side", 20, min(10.0,cycle*4), "flat", False, "V8 gait/contact mechanics with the accepted neutral-chain and lower-foot calibration."),
        PreviewSpec("jaw_turn_front", "Jaw centering under left turn · front", "PROC_TURN_LEFT_35_V8_1_ROOTMOTION", "front", 16, None, "flat", False, "Front geometric review of the rest-local jaw hinge under head yaw and roll."),
        PreviewSpec("power_side", "Airborne power attack · side", "PROC_POWER_ATTACK_V8_1", "side", 24, None, "flat", False, "One-foot launch, 0.196-second flight, target clamp, asymmetric landing and weight arrest."),
        PreviewSpec("power_threeq", "Airborne power attack · three-quarter", "PROC_POWER_ATTACK_V8_1", "threeq", 24, None, "flat", False, "Gameplay view of the same full-body power attack."),
        PreviewSpec("power_sequence", "Walk → power attack → feeding", ("PROC_WALK_RELAXED_V8_1_INPLACE", "PROC_WALK_TO_POWER_ATTACK_V8_1", "PROC_POWER_ATTACK_V8_1", "PROC_POWER_ATTACK_TO_EAT_V8_1", "PROC_EAT_LOOP_V8_1"), "threeq", 20, 10.0, "flat", False, "Ten-second authored sequence with support-aware entry and exact feeding handoff."),
    ]
    ...
    create_master_montage(montages, output / "tarbosaurus_v8_1_master_montage.jpg")
    ...
    html_path = output / "OPEN_ME_tarbosaurus_v8_1_validation.html"
