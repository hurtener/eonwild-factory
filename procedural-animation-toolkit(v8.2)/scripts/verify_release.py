...
def main():
 ...
 result={"schema":"eonwild.v8_2.release_verify.v1","status":"PASS" if exact and changed else "FAIL","baseSha256":sha(base),"candidateSha256":sha(candidate),"changedRotationAccessors":changed,"allowedNodes":sorted(allowed),"checks":{"baseHash":sha(base)==c["baseSha256"],"candidateHash":sha(candidate)==c["outputSha256"],"jsonByteEquivalent":b.json==x.json,"onlyAllowedWalkRotationAccessors":exact,"nonemptyApprovedChangeSet":bool(changed)}}; a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(result,indent=2)+"\n"); print(json.dumps(result,indent=2)); raise SystemExit(0 if result["status"]=="PASS" else 1)
