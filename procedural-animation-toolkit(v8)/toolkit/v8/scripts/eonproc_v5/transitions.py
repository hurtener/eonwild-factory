"""Endpoint-exact, derivative-aware V5 transition synthesis."""
...
def pose_transition(name,source,target,duration,sample_hz,source_phase=0.,target_phase=0.,root_name=None,tags=()):
    ...
    for node in sorted(set(source.translations)|set(target.translations)):
        sv=source.translations.get(node,target.translations[node]); tv=target.translations.get(node,source.translations[node]); p0=sv[si].copy(); p1=tv[ti].copy()
...
def phase_matched_loop_transition(name,source,target,duration,sample_hz,root_name,tags=()):
    ...
    rest=source.translations[root_name][0]; tr={root_name:np.repeat(rest[None,:],count,axis=0)}
    return AnimationClip(name,times,{b:np.asarray(v) for b,v in rotations.items()},tr,{"generator":"Eonwild V5","version":"5.0.0","loop":False,"rootMotion":False,"transition":True,"transitionType":"phase-matched C2 loop blend","sourceClip":source.name,"targetClip":target.name,"sourcePhase":0.,"targetPhase":1.,"exactRuntimeHandoff":True,"tags":list(tags)},{"phase_matched":True,"exact_endpoints":True}).normalize()
