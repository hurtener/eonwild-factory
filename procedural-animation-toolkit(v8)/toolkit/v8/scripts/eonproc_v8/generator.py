...
class TarbosaurusV8Generator:
    ...
    def generate_all(self) -> list[AnimationClip]:
        ...
        terrain_root, terrain_inplace = self._add_locomotion(constant_schedule(
            "PROC_WALK_UNEVEN_TERRAIN_V8_ROOTMOTION",
            g,
            cycles=2.0,
            terrain=uneven,
        ...
        slope_root, slope_inplace = self._add_locomotion(constant_schedule(
            "PROC_WALK_UPSLOPE_V8_ROOTMOTION",
            g,
            cycles=2.0,
            terrain=slope,
        ...
        turn_left_root, turn_left_inplace = self._add_locomotion(constant_schedule(
            "PROC_TURN_LEFT_35_V8_ROOTMOTION",
            g,
            cycles=2.0,
        ...
        turn_right_root, turn_right_inplace = self._add_locomotion(constant_schedule(
            "PROC_TURN_RIGHT_35_V8_ROOTMOTION",
            g,
            cycles=2.0,
            path_factory=arc_factory(-1.0),
            terrain=flat,
        ...
        stop_root, stop_inplace = self._add_locomotion(speed_transition_schedule(
            "PROC_BRAKE_TO_IDLE_V8_ROOTMOTION",
            g,
        ...
        critical = [
            ...
            if gates and (
                ...
                or any(token in name for token in ("IDLE_BREATH", "ALERT_IDLE", "EAT_LOOP", "BITE_ATTACK", "ROAR_V8"))
                or "_TO_" in name
            )
        ]
        ...
        self.report = {
            ...
            "implementation": {
                ...
                "reference_staged_two_step_power_attack": True,
                "roar": True,
    ...
    def manifest(self, asset_name: str) -> dict[str, Any]:
        return {
            "schema": "eonwild.animation-manifest.v8",
            "version": "8.0.0",
            "asset": asset_name,
            ...
            "runtimeContracts": {
                ...
                "powerAttack": "target acquisition -> hindlimb coil -> two catch steps -> late gape -> snap -> clamped tear -> recoil",
            ...
            "stateGraph": {
                "initial": "idle",
                "states": {
                    "idle": {"clip": "PROC_IDLE_BREATH_V8"},
                    "walk": {"clip": "PROC_WALK_RELAXED_V8_INPLACE", "speedMps": self.locomotion.speed},
                    "alert_idle": {"clip": "PROC_ALERT_IDLE_V8"},
                    "alert_walk": {"clip": "PROC_ALERT_WALK_V8_INPLACE"},
                    "eat": {"clip": "PROC_EAT_LOOP_V8"},
                },
                "oneShots": {
                    "bite_left": {"clip": "PROC_BITE_ATTACK_V8", "priority": 70},
                    "bite_right": {"clip": "PROC_BITE_ATTACK_MIRRORED_V8", "priority": 70},
                    "roar": {"clip": "PROC_ROAR_V8", "priority": 60},
                },
                "actionSequences": {
                    "bite_left": ["PROC_WALK_TO_BITE_READY_V8", "PROC_BITE_ATTACK_V8", "PROC_BITE_TO_WALK_V8", "PROC_WALK_RELAXED_V8_INPLACE"],
                    "bite_right": ["PROC_WALK_TO_BITE_READY_MIRRORED_V8", "PROC_BITE_ATTACK_MIRRORED_V8", "PROC_BITE_MIRRORED_TO_WALK_V8", "PROC_WALK_RELAXED_V8_INPLACE"],
                    "roar": ["PROC_WALK_TO_ROAR_READY_V8", "PROC_ROAR_V8", "PROC_ROAR_TO_WALK_V8", "PROC_WALK_RELAXED_V8_INPLACE"],
                    "eat": ["PROC_WALK_TO_EAT_V8", "PROC_EAT_LOOP_V8", "PROC_EAT_TO_WALK_V8", "PROC_WALK_RELAXED_V8_INPLACE"],
                    "alert": ["PROC_WALK_TO_ALERT_WALK_V8", "PROC_ALERT_WALK_V8_INPLACE"],
                },
            },
        }
    ...
    def write(self, output_dir: str | Path, source_path: str | Path, bone_map_path: str | Path):
        ...
        if not self.clips:
            self.generate_all()
        glb_path = output / "tarbosaurus_procedural_v8_animation_pack.glb"
        self.asset.write_animations(glb_path, self.animation_specs(), replace_existing=True)
        manifest = self.manifest(glb_path.name)
        manifest_path = output / "animation-manifest.v8.json"
        manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
        profile_path = output / "resolved-profile-v8.json"
        ...
        report_path = output / "procedural-v8-report.json"
