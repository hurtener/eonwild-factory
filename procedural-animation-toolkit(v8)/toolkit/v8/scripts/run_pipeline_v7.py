#!/usr/bin/env python3
"""Generate the complete Eonwild Tarbosaurus V7 animation pack."""
...
from eonproc_v7.generator import TarbosaurusV7Generator
...
def main() -> int:
    ...
    profile = BipedV7Profile.from_json(args.profile) if args.profile else BipedV7Profile()
    jaw_meta = None
    ...
    generator = TarbosaurusV7Generator(asset, semantics, profile)
    generator.generate_all()
    outputs = generator.write(args.output_dir, args.input, args.bone_map)
    if jaw_meta is not None:
        jaw_path = Path(args.output_dir) / "jaw-calibration-v7.json"
