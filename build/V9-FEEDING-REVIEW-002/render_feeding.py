...
def import_character(path,clip,frames,fps):
    ...
    # Its oral endpoint remains bound to the emitted lower-oral witness.
    ...
    for sample in receipt['samples']:
        ...
        end=Vector((lower.x,-lower.z,lower.y))
        ...
        coupling.append({'time_seconds':t,'food_gain':food_gain,'food_center_canonical':list(location),
                         'connector_lower_oral_endpoint_canonical':list(lower),'prop_yield_m':yield_m,
                         'authority':'AUTHORED_NOT_PHYSICS'})
    (path.parent/'prop-coupling-receipt.json').write_text(json.dumps({'samples':coupling},indent=2)+'\n')
    timeline['previewTargetProp']['behavior']='C1 ground-preserving yield; oral-witness tissue; held fragment proxy during grip and post-release chew'
