#!/usr/bin/env python3
"""Render the baked V5 GLB and build a self-contained multi-clip validation HTML."""
...
class V5Renderer:
    def __init__(
        ...
        texture_path = output_dir / f"_v5_basecolor{extension}"
...
def render_preview(
    ...
    video = output_dir / f"v5_{spec.key}_{spec.fps}fps.mp4"
    ...
    montage = output_dir / f"v5_{spec.key}_keyframes.jpg"
...
def create_html(destination: Path, media: dict, specs: list[PreviewSpec], report: dict, fixture: dict, baked: dict | None) -> None:
    ...
    locomotion_clip = report_clips.get("PROC_WALK_RELAXED_V5_ROOTMOTION", {})
    locomotion_diag = locomotion_clip.get("diagnostics", {})
    terrain_diag = report_clips.get("PROC_WALK_UNEVEN_TERRAIN_V5_ROOTMOTION", {}).get("diagnostics", {})
    ...
    html = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Eonwild Tarbosaurus V5 validation</title>
<style>
...
</style></head><body><div class="wrap"><header><div><div class="eyebrow">Eonwild procedural animation toolkit · V5</div><h1>Tarbosaurus production animation pack</h1></div><div class="status">✓ generated on the real 75-joint rig</div></header>
...
def main() -> int:
    ...
    renderer = V5Renderer(asset, generator, args.width, args.height, output)

    cycle = 1.0 / profile.cycle_hz
    alert_cycle = 1.0 / profile.alert_cycle_hz
    specs = [
        PreviewSpec("walk_side", "V5 relaxed walk · closed living-neutral mouth", "PROC_WALK_RELAXED_V5_INPLACE", "side", 60, cycle, "flat", False, "Review closed-mouth living neutral, support transfer, spine/head mass, and softer distal tail."),
        PreviewSpec("walk_threeq", "V5 relaxed walk · three-quarter", "PROC_WALK_RELAXED_V5_INPLACE", "threeq", 60, cycle, "flat", False, "The same polished baked track from the gameplay camera."),
        PreviewSpec("idle", "Living-neutral idle", "PROC_IDLE_BREATH_V5", "threeq", 30, 6.0, "flat", False, "The mouth remains closed while breathing, weight shift, head drift, arms, and tail retain subtle life."),
        PreviewSpec("eat", "Eating loop · full closure between bites", "PROC_EAT_LOOP_V5", "threeq", 30, 8.0, "flat", False, "Four staged feeding bites with full neutral closure between cycles and polished whole-body recovery."),
        PreviewSpec("bite_transition", "Walk → bite → walk · exact authored handoffs", ("PROC_WALK_RELAXED_V5_INPLACE", "PROC_WALK_TO_BITE_READY_V5", "PROC_BITE_ATTACK_V5", "PROC_BITE_TO_WALK_V5", "PROC_WALK_RELAXED_V5_INPLACE"), "threeq", 30, None, "flat", False, "Actual baked sequence. The bite closes fully at contact; transition endpoints are handed off without a second runtime crossfade."),
        PreviewSpec("roar_transition", "Walk → roar → walk · exact authored handoffs", ("PROC_WALK_RELAXED_V5_INPLACE", "PROC_WALK_TO_ROAR_READY_V5", "PROC_ROAR_V5", "PROC_ROAR_TO_WALK_V5", "PROC_WALK_RELAXED_V5_INPLACE"), "threeq", 30, None, "flat", False, "Large intentional gape for the roar, followed by complete return to the calibrated living-neutral closure."),
    ]

    videos: dict[str, Path] = {}
    montages: list[tuple[PreviewSpec, Path]] = []
    for spec in specs:
        print(f"[render-v5] {spec.key}: {spec.label}", file=sys.stderr, flush=True)
    ...
    create_master_montage(montages, output / "tarbosaurus_v5_master_montage.jpg")

    report = json.loads(Path(args.report).read_text(encoding="utf-8"))
    fixture_path = output.parent / "v5-fixture-validation.json"
    fixture = json.loads(fixture_path.read_text()) if fixture_path.exists() else {}
    baked_path = output / "v5-baked-glb-validation.json"
    ...
    html_path = output / "OPEN_ME_tarbosaurus_v5_validation.html"
