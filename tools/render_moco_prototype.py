...
def draw(t):
    ...
    axial=[n for n in body if n.startswith(('tail_','neck')) or n=='head']
    for name in axial:
        b=body[name];s,e=xy(b['origin']),xy(b['end']);width=max(4,meta['segments'][name]['radius_m']*65)
        ax.plot([s[0],e[0]],[s[1],e[1]],color='#8db5ab',lw=width,solid_capstyle='round',alpha=.85)
        ax.scatter(*s,s=27,color='#def5eb',zorder=4)
    h=xy(body['head' if 'head' in body else 'neck']['end']);ax.plot([h[0]-.15,h[0]+.4],[h[1],h[1]-.02],color='#aac8bb',lw=20,solid_capstyle='round')
    ...
    for suffix in (['r','l'] if half%2==0 else ['l','r']):
        ...
        for part,width in [('thigh',16),('shin',10),('metatarsus',7),('toe',5)]+([('digit',4)] if 'digit_'+suffix in body else []):
...
for name,col in [('pitch','#ecba78'),('tail_0' if meta.get('spatial') else 'tail_proximal','#b2dbaa'),('tail_3' if meta.get('spatial') else 'tail_distal','#7eaccc')]:axes[1,1].plot(t,[np.degrees(r['coordinates'][name]['value']) for r in frames],color=col,label=name.replace('_',' '))
