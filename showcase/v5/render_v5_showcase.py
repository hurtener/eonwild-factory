#!/usr/bin/env python3
"""Render the validated Tarbosaurus V5 GLB as a local showcase.
...
FILM_NAMES = (
    "PROC_IDLE_BREATH_V5",
    "PROC_WALK_RELAXED_V5_INPLACE",
    "PROC_TURN_LEFT_35_V5_INPLACE",
    "PROC_START_WALK_V5_INPLACE",
    "PROC_EAT_LOOP_V5",
    "PROC_BITE_ATTACK_V5",
    "PROC_ROAR_V5",
)
...
def world_setup() -> None:
    world = bpy.data.worlds.new("V5 Showcase World") if not bpy.data.worlds else bpy.data.worlds[0]
...
def safe_slug(name: str) -> str:
    return name.lower().replace("proc_", "").replace("_v5", "").replace("_", "-")
