#!/usr/bin/env python3
"""Render the validated Tarbosaurus V8.1 GLB as a local showcase.
...
FILM_NAMES = (
    "PROC_IDLE_BREATH_V8_1",
    "PROC_WALK_RELAXED_V8_1_INPLACE",
...
def args() -> argparse.Namespace:
    ...
    p.add_argument("--view", choices=("side","threeq","front"), default="side")
    p.add_argument("--power-sequence", action="store_true", help="Render a 10-second walk to power attack to feeding film")
    p.add_argument("--sequence-only", action="store_true", help="Skip clip stills/films and render only the power sequence")
...
def make_stage(bounds: tuple[Vector, Vector], view: str="side") -> tuple[bpy.types.Camera, bpy.types.Object]:
    lo, hi = bounds
    center = (lo + hi) * 0.5
    size = hi - lo
    ...
    # Project-owned review proxy: a deliberately abstract low carcass/target
    # makes bite contact and feeding continuity readable without importing any
    # protected reference art. It is evidence staging, never part of the GLB.
    target_location = (
        center.x,
        center.y - size.y * 0.27,
        lo.z + size.z * 0.055,
    )
    bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=3, radius=1.0, location=target_location)
    target = bpy.context.object
    target.name = "PROJECT_OWNED_TARGET_PROXY"
    target.scale = (size.x * 0.10, size.y * 0.075, size.z * 0.055)
    target.data.materials.append(material("Target proxy", (0.16, 0.035, 0.025, 1.0), 0.92))
    ...
    if view=="front": camera_location=(center.x,center.y-size.y*1.7,center.z+size.z*.40)
    elif view=="threeq": camera_location=(center.x+camera_distance*.78,center.y-size.y*.72,center.z+size.z*.48)
    else: camera_location=(center.x+camera_distance,center.y,center.z+size.z*.48)
...
def world_setup() -> None:
    world = bpy.data.worlds.new("V8.1 Showcase World") if not bpy.data.worlds else bpy.data.worlds[0]
...
def safe_slug(name: str) -> str:
    return name.lower().replace("proc_", "").replace("_v8_1", "").replace("_", "-")
...
def render_power_sequence(scene,actions,clip_by_name,armature,path,fps):
    segments=(("PROC_WALK_RELAXED_V8_1_INPLACE",1.0),("PROC_WALK_TO_POWER_ATTACK_V8_1",.42),("PROC_POWER_ATTACK_V8_1",2.8),("PROC_POWER_ATTACK_TO_EAT_V8_1",.82),("PROC_EAT_LOOP_V8_1",4.96))
    frame_dir=path.parent/f".{path.stem}.frames";frame_dir.mkdir(parents=True,exist_ok=True); scene.render.image_settings.file_format="PNG"; scene.render.fps=fps; output_index=1
    for name,duration in segments:
        action=actions[name]; start,end=set_action(armature,action); source=max(1,end-start); count=round(duration*fps)
        for local in range(count):
            phase=local/max(count-1,1); scene.frame_set(start+int(round(source*phase))); scene.render.filepath=str(frame_dir/f"frame_{output_index:04d}.png"); bpy.ops.render.render(write_still=True); output_index+=1
    ffmpeg=shutil.which("ffmpeg"); subprocess.run([ffmpeg,"-y","-hide_banner","-loglevel","error","-framerate",str(fps),"-start_number","1","-i",str(frame_dir/"frame_%04d.png"),"-frames:v",str(output_index-1),"-vf","pad=ceil(iw/2)*2:ceil(ih/2)*2","-c:v","libx264","-pix_fmt","yuv420p","-movflags","+faststart",str(path)],check=True); shutil.rmtree(frame_dir)


POWER_SEQUENCE_SEGMENTS = [
    {"clip": "PROC_WALK_RELAXED_V8_1_INPLACE", "duration_seconds": 1.0},
    {"clip": "PROC_WALK_TO_POWER_ATTACK_V8_1", "duration_seconds": 0.42},
    {"clip": "PROC_POWER_ATTACK_V8_1", "duration_seconds": 2.8},
    {"clip": "PROC_POWER_ATTACK_TO_EAT_V8_1", "duration_seconds": 0.82},
    {"clip": "PROC_EAT_LOOP_V8_1", "duration_seconds": 4.96},
]
...
def main() -> None:
    ...
    camera, _ground = make_stage(bounds,cfg.view)
    world_setup()
    ...
    if not cfg.film_name and not cfg.sequence_only:
        for clip in priority:
            name = clip["name"]
    ...
    if cfg.film_name and cfg.film_name in actions:
        render_still(
            scene,
            camera,
            actions[cfg.film_name],
            armature,
            still_dir / f"{safe_slug(cfg.film_name)}.png",
        )
    if not cfg.still_only and not cfg.sequence_only:
    ...
    if cfg.power_sequence:
        render_power_sequence(scene,actions,{c['name']:c for c in manifest['clips']},armature,film_dir/f"power-sequence-{cfg.view}.mp4",cfg.fps)
    ...
    report = {
        ...
        "fps": cfg.fps,
        "view": cfg.view,
        "source_glb_sha256": __import__('hashlib').sha256(cfg.input.read_bytes()).hexdigest(),
        "priority_clip_count": len(priority),
        "priority_clips": [c["name"] for c in priority],
        "film_clips": (
            [] if cfg.sequence_only or cfg.still_only
            else [name for name in film_names if name in actions]
        ),
        "power_sequence_rendered": bool(cfg.power_sequence),
        "power_sequence_segments": POWER_SEQUENCE_SEGMENTS if cfg.power_sequence else [],
        "power_sequence_duration_seconds": (
            sum(segment["duration_seconds"] for segment in POWER_SEQUENCE_SEGMENTS)
            if cfg.power_sequence else None
        ),
        "review_proxy_policy": (
            "project-owned abstract primitive at the contact/feeding point; evidence only; not part of the GLB"
            if cfg.power_sequence else None
        ),
