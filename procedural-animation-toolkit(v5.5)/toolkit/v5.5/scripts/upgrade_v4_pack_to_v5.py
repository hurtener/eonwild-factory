#!/usr/bin/env python3
"""Build V5 by refining the already validated V4 contact/terrain solution.
...
from eonproc_v5.transitions import pose_transition, phase_matched_loop_transition
...
def rename_v5(name): return name.replace('_V4','_V5').replace('V4_','V5_')
...
def jaw_curve(name,times,neutral):
    ...
    if 'BITE_ATTACK' in name:
        close=np.full_like(u,neutral); a=.28; contact=.56; recovery=.80
        for i,x in enumerate(u):
            if x<a: gape=49.*minimum_jerk(x/a)
...
def apply_detail(asset,sem,basis,clip):
    ...
    clip.extras.update({'generator':'Eonwild procedural animation toolkit v5','version':'5.0.0','polished':True,'contactTracksPreserved':True,'polishWindowFrames':7})
...
def main():
    ...
    for a in reader.animations.values():
        ...
        c=baked_to_clip(a); c.name=rename_v5(c.name); c.extras={**c.extras,'version':'5.0.0','generator':'Eonwild procedural animation toolkit v5','sourceVersion':'4.0.0'}
    ...
    transitions=[
      pt('PROC_IDLE_TO_WALK_V5','PROC_IDLE_BREATH_V5','PROC_WALK_RELAXED_V5_INPLACE',1.95,0,0,('transition','idle','walk')),
      pt('PROC_WALK_TO_IDLE_V5','PROC_WALK_RELAXED_V5_INPLACE','PROC_IDLE_BREATH_V5',1.95,.5,0,('transition','walk','idle')),
      phase_matched_loop_transition('PROC_WALK_TO_ALERT_WALK_V5',by['PROC_WALK_RELAXED_V5_INPLACE'],by['PROC_ALERT_WALK_V5_INPLACE'],2.2222222,SAMPLE_HZ,root,('transition','walk','alert')),
      phase_matched_loop_transition('PROC_ALERT_WALK_TO_WALK_V5',by['PROC_ALERT_WALK_V5_INPLACE'],by['PROC_WALK_RELAXED_V5_INPLACE'],2.2222222,SAMPLE_HZ,root,('transition','alert','walk')),
      pt('PROC_IDLE_TO_ALERT_IDLE_V5','PROC_IDLE_BREATH_V5','PROC_ALERT_IDLE_V5',1.4,0,0,('transition','idle','alert')),
      pt('PROC_ALERT_IDLE_TO_IDLE_V5','PROC_ALERT_IDLE_V5','PROC_IDLE_BREATH_V5',1.4,0,0,('transition','alert','idle')),
      pt('PROC_IDLE_TO_EAT_V5','PROC_IDLE_BREATH_V5','PROC_EAT_LOOP_V5',2.05,0,0,('transition','idle','eat')),
      pt('PROC_EAT_TO_IDLE_V5','PROC_EAT_LOOP_V5','PROC_IDLE_BREATH_V5',2.05,0,0,('transition','eat','idle')),
      pt('PROC_IDLE_TO_ROAR_READY_V5','PROC_IDLE_BREATH_V5','PROC_ROAR_V5',.95,0,0,('transition','idle','roar')),
      pt('PROC_ROAR_TO_IDLE_V5','PROC_ROAR_V5','PROC_IDLE_BREATH_V5',1.55,1,0,('transition','roar','idle')),
      pt('PROC_WALK_TO_BITE_READY_V5','PROC_WALK_RELAXED_V5_INPLACE','PROC_BITE_ATTACK_V5',.95,.5,0,('transition','walk','bite','urgent')),
      pt('PROC_BITE_TO_WALK_V5','PROC_BITE_ATTACK_V5','PROC_WALK_RELAXED_V5_INPLACE',1.55,1,0,('transition','bite','walk','recovery')),
      pt('PROC_WALK_TO_ROAR_READY_V5','PROC_WALK_RELAXED_V5_INPLACE','PROC_ROAR_V5',1.45,.5,0,('transition','walk','roar')),
      pt('PROC_ROAR_TO_WALK_V5','PROC_ROAR_V5','PROC_WALK_RELAXED_V5_INPLACE',2.0,1,0,('transition','roar','walk','recovery')),
      pt('PROC_WALK_TO_EAT_V5','PROC_WALK_RELAXED_V5_INPLACE','PROC_EAT_LOOP_V5',2.1,.5,0,('transition','walk','eat','settle')),
      pt('PROC_EAT_TO_WALK_V5','PROC_EAT_LOOP_V5','PROC_WALK_RELAXED_V5_INPLACE',2.1,0,0,('transition','eat','walk','rise')),
    ]
    clips=base+transitions
    specs=[c.gltf_spec() for c in clips]; glb=out/'tarbosaurus_procedural_v5_animation_pack.glb'; asset.write_animations(glb,specs,True)
    manifest={'schema':'eonwild.animation-manifest.v5','version':'5.0.0','asset':glb.name,'family':'large_theropod_biped','sampleHz':60,'jawCalibration':jaw.to_dict(),'clips':[{'name':c.name,'durationSeconds':c.duration,'loop':bool(c.extras.get('loop',False)),'rootMotion':bool(c.extras.get('rootMotion',False)),'tags':c.extras.get('tags',[]),'terrain':c.extras.get('terrain'),'path':c.extras.get('path'),'transition':bool(c.extras.get('transition',False)),'sourceClip':c.extras.get('sourceClip'),'targetClip':c.extras.get('targetClip'),'sourcePhase':c.extras.get('sourcePhase'),'targetPhase':c.extras.get('targetPhase'),'exactRuntimeHandoff':c.extras.get('exactRuntimeHandoff',False)} for c in clips], 'runtimeContracts':{'authoredTransitionHandoff':'switch exactly at transition endpoint; do not add another crossfade','directClipSelection':'snapshot blend with root continuity','neutralJaw':'apply calibrated living-neutral closure to all non-vocal/action states'},'actionSequences':{'bite':['PROC_WALK_TO_BITE_READY_V5','PROC_BITE_ATTACK_V5','PROC_BITE_TO_WALK_V5','PROC_WALK_RELAXED_V5_INPLACE'],'roar':['PROC_WALK_TO_ROAR_READY_V5','PROC_ROAR_V5','PROC_ROAR_TO_WALK_V5','PROC_WALK_RELAXED_V5_INPLACE'],'eat':['PROC_WALK_TO_EAT_V5','PROC_EAT_LOOP_V5','PROC_EAT_TO_WALK_V5','PROC_WALK_RELAXED_V5_INPLACE'],'alert':['PROC_WALK_TO_ALERT_WALK_V5','PROC_ALERT_WALK_V5_INPLACE']}}
    (out/'animation-manifest.v5.json').write_text(json.dumps(manifest,indent=2))
    v4_report=json.load(open(args.v4_report)) if args.v4_report and Path(args.v4_report).exists() else None
    report={'status':'PASS_WITH_PERCEPTUAL_REVIEW_REQUIRED','version':'5.0.0','buildMode':'contact-preserving refinement of validated V4 tracks','jawCalibration':jaw.to_dict(),'clipCount':len(clips),'baseClipCount':len(base),'transitionClipCount':len(transitions),'preserved':{'skinJointCount':len(asset.skin_data()[0]),'vertexSoleAndTerrainTracks':True,'anatomicalLegConstraintTracks':True,'rootMotionTracks':True},'fixes':{'neutralJawCalibrated':True,'biteReopenBugRemoved':True,'secondaryQuaternionPolish':True,'allTransitionsRebuilt':True,'playerExactHandoffContract':True},'input':{'sourceGlb':str(Path(args.source_glb).resolve()),'sourceSha256':sha256_file(args.source_glb),'boneMap':str(Path(args.bone_map).resolve()),'boneMapSha256':sha256_file(args.bone_map)},'output':{'glb':str(glb.resolve()),'glbSha256':sha256_file(glb)},'sourceV4QualityGates':None if v4_report is None else v4_report.get('quality_gate_summary')}
    (out/'procedural-v5-report.json').write_text(json.dumps(report,indent=2))
    (out/'resolved-profile-v5.json').write_text(json.dumps({'name':'tarbosaurus_v5_contact_preserving_polish','sampleHz':60,'neutralJawCloseDegrees':neutral,'jawBreathDegrees':.36,'polishWindowFrames':7,'transitionSystem':'RotationSpline + Hermite exact handoff'},indent=2))
    print(json.dumps({'glb':str(glb),'report':str(out/'procedural-v5-report.json'),'manifest':str(out/'animation-manifest.v5.json'),'jaw':jaw.to_dict(),'clips':len(clips)},indent=2))
