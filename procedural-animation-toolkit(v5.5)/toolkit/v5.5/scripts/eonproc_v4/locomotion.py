...
class TerrainAwareLocomotionGenerator(AnatomicallyConstrainedBipedGenerator):
    ...
    def _leg_preferences(self, foot: V4FootPlan, global_phase: float) -> tuple[float, float, float]:
        ...
        # The knee leads the recovery and the ankle follows from a slightly
        # straighter loaded state. These offsets keep the chain phased instead
        # of folding every joint at the same instant.
    ...
    def generate_schedule(self, schedule: LocomotionSchedule, keep_world_matrices: bool = True) -> LocomotionResult:
        ...
        pelvis_translation: list[np.ndarray] = []
        ...
        for index, (time_value, phase, body) in enumerate(zip(schedule.times, schedule.phases, body_plans)):
            ...
            pelvis_translation.append(pose.translation[self.asset.name_to_node[self.pelvis]].copy())
            for side in ("l", "r"):
        ...
        pelvis_array = np.asarray(pelvis_translation)
        if schedule.loop:
            pelvis_array[-1] = pelvis_array[0]
        ...
        pelvis_export = pelvis_array[indices]
        ...
        clip = AnimationClip(
            ...
            translations={self.root: root_export, self.pelvis: pelvis_export}, extras=extras,
        ...
        if schedule.root_motion:
            ...
            in_place = AnimationClip(
                ...
                translations={self.root: in_place_root, self.pelvis: pelvis_export.copy()}, extras=in_place_extras,
