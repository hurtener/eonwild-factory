...
class ActionGenerator:
    ...
    def _fixed_foot_plan(self, side: str, load: float, forward_offset: float = 0.0) -> V4FootPlan:
        target = self.g.foot_rest[side].copy()
        target += self.g.base_basis.forward * float(forward_offset)
        return V4FootPlan(
    ...
    def _pose(
        ...
        for side, load, forward_offset in (
            ("l", controls.left_load, controls.left_foot_forward),
            ("r", controls.right_load, controls.right_foot_forward),
        ):
            foot = self._fixed_foot_plan(side, load, forward_offset)
    ...
    def idle(self) -> AnimationClip:
        ...
        def controls(t: float, u: float) -> ActionControls:
            ...
            return ActionControls(
                ...
                # Same-model neutral reference: one leg supports beneath the
                # pelvis while the other rests forward to lengthen the support
                # polygon. Keep the load bias stable instead of swapping the
                # support role every half-cycle.
                left_load=0.66 + 0.035 * math.sin(cycle),
                right_load=0.34 - 0.035 * math.sin(cycle),
                left_foot_forward=-self.g.hip_height * 0.030,
                right_foot_forward=self.g.hip_height * 0.320,
            )
    ...
    def alert_idle(self) -> AnimationClip:
        ...
        def controls(t: float, u: float) -> ActionControls:
            ...
            return ActionControls(
                ...
                left_foot_forward=-self.g.hip_height * 0.025,
                right_foot_forward=self.g.hip_height * 0.120,
            )
    ...
    def eating_loop(self) -> AnimationClip:
        ...
        def controls(t: float, u: float) -> ActionControls:
            cycle = 2.0 * math.pi * u
            # A slow, loop-safe whole-body settle makes the leg compression
            # precede and support the feeding reach.  The prior constant drop
            # was numerically present but invisible as an action.
            settle = 0.55 + 0.45 * (0.5 - 0.5 * math.cos(cycle))
            ...
            return ActionControls(
                pelvis_translation=-self.g.base_basis.forward * (self.g.hip_height * self.profile.eating_pelvis_back_hip_fraction * settle)
                    - self.g.base_basis.up * (self.g.hip_height * 0.110 * settle)
                    + self.g.base_basis.lateral * (self.g.hip_height * 0.008 * math.sin(cycle)),
                pelvis_pitch=-math.radians(1.8) * settle,
                spine_pitch=math.radians(self.profile.eating_body_lower_deg) * 0.72 * settle,
                neck_pitch=math.radians(self.profile.eating_neck_lower_deg) * (0.72 + 0.28 * settle) + math.radians(1.5) * math.sin(2.0 * cycle),
                neck_yaw=side_search * 0.35,
                head_pitch=math.radians(self.profile.eating_head_lower_deg) * (0.72 + 0.28 * settle) + math.radians(1.0) * math.sin(2.0 * cycle + 0.3),
                ...
                tail_pitch=-math.radians(7.5) * settle,
                ...
                left_foot_forward=-self.g.hip_height * 0.030,
                right_foot_forward=self.g.hip_height * 0.140,
            )
    ...
    def bite_attack(self) -> AnimationClip:
        ...
        def controls(t: float, u: float) -> ActionControls:
            ...
            active = (1.0 - recovery)
            lunge = attack * active
            brace = anticipation * (1.0 - attack) * active
            crouch = brace + 0.28 * lunge
            # Keep the mouth quiet through the first half of anticipation so
            # the rear/down leg load is the first readable event.
            jaw_prepare_start = a * 0.48
            jaw_prepare = minimum_jerk(max(0.0, min(1.0, (u - jaw_prepare_start) / max(a - jaw_prepare_start, 1e-6))))
            jaw_open = math.radians(self.profile.bite_jaw_open_deg) * jaw_prepare * (1.0 - minimum_jerk(max(0.0, (u - (c - 0.07)) / 0.07)))
            ...
            return ActionControls(
                pelvis_translation=-self.g.base_basis.forward * (self.g.hip_height * self.profile.bite_rear_load_hip_fraction * brace)
                    - self.g.base_basis.up * (self.g.hip_height * (0.082 * brace + 0.014 * lunge))
                    + self.g.base_basis.forward * (self.g.hip_height * 0.035 * lunge),
                pelvis_pitch=-math.radians(4.2) * brace + math.radians(1.1) * lunge,
                spine_pitch=-math.radians(self.profile.bite_body_crouch_deg) * brace - math.radians(1.2) * lunge + math.radians(2.0) * recoil,
                ...
                head_pitch=-math.radians(3.0) * lunge + math.radians(3.0) * recoil,
                head_yaw=math.radians(self.profile.bite_head_yaw_deg) * math.sin(math.pi * attack) * active,
                jaw_open=jaw_open,
                tail_pitch=-math.radians(7.0) * brace - math.radians(3.0) * lunge + math.radians(2.0) * recoil,
                tail_yaw=-math.radians(3.0) * math.sin(math.pi * attack) * active,
                arm_tension=anticipation * active,
                root_forward=self.g.hip_height * (-self.profile.bite_rear_load_hip_fraction * brace + self.profile.bite_lunge_hip_fraction * lunge * (1.0 - 0.72 * recovery)),
                left_load=0.55 + 0.18 * brace - 0.04 * lunge,
                right_load=0.45 - 0.18 * brace + 0.04 * lunge,
                left_foot_forward=-self.g.hip_height * 0.040,
                right_foot_forward=self.g.hip_height * 0.160,
            )
