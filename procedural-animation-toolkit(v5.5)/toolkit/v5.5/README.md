# Eonwild Procedural Animation V5.5

V5.5 is a full constrained-solver balance and posture correction pass. It is an independent copy and iteration lane; the original `procedural-animation-toolkit(v5)` repository directory is immutable.

## V5.5 balance corrections

- A calmer neutral body pitch with one supporting leg beneath the mass and the opposite foot forward.
- Exported pelvis translations for locomotion and actions, including in-place clips.
- Shorter rear stance and hip-led recovery with delayed knee, ankle, foot, and toe response.
- Rear-loaded, leg-driven bite delivery instead of a head-first lunge.
- Pelvis-and-knee feeding descent with bounded torso pitch and tail counterbalance.
- A full regeneration from the source GLB before applying V5 jaw calibration, polish, and transitions.

Reproduce and validate from the toolkit root:

```bash
PYTHON_BIN=.venv/bin/python ./scripts/reproduce_v5_5.sh
PYTHON_BIN=.venv/bin/python ./scripts/run_v5_5_tests.sh
```

The release is under `validated_result/v5.5/`. Its visual balance measurements are skeleton-based proxies, not a physical center-of-mass or biological validation.

## Inherited V5 architecture
